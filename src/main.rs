...
fn main() -> Result<(), DynError> {
    ...
    // for debug
    let _logger = Logger::new("robo2_2_1_2024_a");
    let ctx = Context::new()?;
    let mut selector = ctx.create_selector()?;
    let node = ctx.create_node("robo2_2_1_2024_a", None, Default::default())?;

    let subscriber_cmd = node.create_subscriber::<msg::Twist>("cmd_vel2_2_1", None)?;
    let subscriber_joy = node.create_subscriber::<sensor_msgs::msg::Joy>("rjoy2_2_1", None)?;
    ...
    selector.add_subscriber(
        ...
        Box::new(move |msg| {
            ...
            for i in motor_power.keys() {
                udp_communication::send_pwm_udp("50005", "192.168.5:60000", *i, motor_power[i]);
            }
        }),
    );
